...
public abstract class AbstractBeanFactory implements BeanFactory {
    ...
    @Override
    public BeanDefinition createBeanDefinition(String className, PropertyValues propertyValues) {
        ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
        try {
            return new BeanDefinition(Class.forName(className, true, classLoader), propertyValues);
        } catch (ClassNotFoundException e) {
            throw new UnsupportedOperationException("Error creating bean with class '" + className + "' not found", e);
        }
    }
}
