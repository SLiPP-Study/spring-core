...
import analyze.beans.MutablePropertyValues;
import analyze.beans.PropertyValue;
import analyze.beans.factory.BeanDefinitionStoreException;
import analyze.beans.factory.config.ConstructorArgumentValues;
import analyze.beans.factory.config.RuntimeBeanReference;
import analyze.beans.factory.support.*;
import analyze.core.io.Resource;
import analyze.util.StringUtils;
...
public class DefaultXmlBeanDefinitionParser implements XmlBeanDefinitionParser {
	...
	public void registerBeanDefinitions(BeanDefinitionRegistry beanFactory, ClassLoader beanClassLoader,
		...
		log.debug("Default lazy init '" + this.defaultLazyInit + "'");
		this.defaultDependencyCheck = root.getAttribute(DEFAULT_DEPENDENCY_CHECK_ATTRIBUTE);
		log.debug("Default dependency check '" + this.defaultDependencyCheck + "'");
		this.defaultAutowire = root.getAttribute(DEFAULT_AUTOWIRE_ATTRIBUTE);
		log.debug("Default autowire '" + this.defaultAutowire + "'");
		...
		log.debug("Found " + beanDefinitionCounter + " <" + BEAN_ELEMENT + "> elements defining beans");
	}
	...
	protected void loadBeanDefinition(Element ele) {
		...
		if (id == null || "".equals(id) && !aliases.isEmpty()) {
			...
			log.debug("No XML 'id' specified - using '" + id + "' as ID and " + aliases + " as aliases");
		}
		...
		if (id == null || "".equals(id)) {
			if (beanDefinition instanceof RootBeanDefinition) {
				...
				log.debug("Neither XML 'id' nor 'name' specified - using bean class name [" + id + "] as ID");
			}
			else {
		...
		log.debug("Registering bean definition with id '" + id + "'" + " , beanDefinition :: {}", beanDefinition);
